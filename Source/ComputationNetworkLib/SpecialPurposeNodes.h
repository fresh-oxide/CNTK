...
public:
    ...
    virtual void /*ComputationNodeNonLooping::*/ ForwardPropNonLooping() override // -sum(left_i * log(softmax_i(right)))
    {
        ...
        m_softmaxOfRight->AddWithScaleOf((ElemType)-1.0, Input(2)->ValueAsMatrix()); //Scaled log likelihood
        Matrix<ElemType>::Scale(m_acweight, *m_softmaxOfRight);
        m_softmaxOfRight->InplaceExp();

        int nf = m_softmaxOfRight->GetNumCols();
        int nstates = m_tmap->GetNumCols();

        //SaveMatrix(L"D:\\temp\\cntk_p_new_betterrepro.txt", m_softmaxOfRight);
        ...
        for (int f = nf; f >= 0; f--) {  // not nf-1 because of transitions to final state at the end of the observation sequence
            ...
            //Matrix<ElemType>::Scale(scale, *m_curr_alpha);
        }
        ...
        //cout << "gradient abs sum:" << Value().SumOfAbsElements() << endl;
        /*m_softmaxOfRight->AssignDifferenceOf((ElemType)1.0, *m_softmaxOfRight);
        Value().ElementMultiplyWith(*m_softmaxOfRight);*/
